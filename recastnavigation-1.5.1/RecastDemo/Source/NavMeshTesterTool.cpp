...
void NavMeshTesterTool::recalc()
{
    ...
    std::ofstream in;
    in.open("./output.txt", std::ios::trunc);
    in << "StarPos:" << m_spos[0] << "," << m_spos[1] << "," << m_spos[2] << "\r\n";
    in << "EndPos:" << m_epos[0] << "," << m_epos[1] << "," << m_epos[2] << "\r\n";
    for (int i = 0; i < m_nsmoothPath; ++i) {
        in << m_smoothPath[i * 3] << "," << m_smoothPath[i * 3 + 1] + 0.1f << "," << m_smoothPath[i * 3 + 2] << "\r\n";
    }
    in.close();//¹Ø±ÕÎÄ¼þ
#endif
}

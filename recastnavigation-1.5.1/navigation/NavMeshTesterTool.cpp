...
bool NavMeshTesterTool::getResultByIndex(int index, float* out)
{
    if (index < 0 || index >= m_nsmoothPath) return false;

    out[0] = m_smoothPath[index * 3];
    out[1] = m_smoothPath[index * 3 + 1] + 0.1f;
    out[2] = m_smoothPath[index * 3 + 2];
    return true;
}
